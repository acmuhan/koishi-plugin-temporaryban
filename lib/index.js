...
function apply(ctx, config) {
    ...
    cmd.subcommand('.add <word:string>', 'Add a forbidden word to current group')
        ...
        if (!session.guildId)
            return 'This command must be used in a group.';
        if (!word)
            return 'Please specify a word.';
        const groupConfig = config.groups.find(g => g.groupId === session.guildId);
        if (!groupConfig)
            return 'This group is not configured for monitoring.';
    ...
    cmd.subcommand('.remove <word:string>', 'Remove a forbidden word')
        ...
        const groupConfig = config.groups.find(g => g.groupId === session.guildId);
        if (!groupConfig)
            return 'This group is not configured.';
    ...
    ctx.on('message', async (session) => {
        ...
        if (!isGroup) {
            if (config.debug)
                logger.debug(`[DEBUG] Ignored: Not a group message.`);
            return;
        }
        if (!session.content ||
            session.selfId === session.userId) {
            if (config.debug)
                logger.debug(`[DEBUG] Ignored: No content or self message.`);
            return;
        }
        const elements = session.elements || [];
        if (!elements.some((el) => el.type === 'text')) {
            if (config.debug)
                logger.debug(`[DEBUG] Ignored: No text element.`);
            return;
        }
        ...
        if (!groupId || !userId) {
            if (config.debug)
                logger.warn(`[DEBUG] Ignored: Missing groupId or userId.`);
            return;
        }
        // Config Check
        ...
        if (!groupConfig) {
            if (config.debug)
                logger.debug(`[DEBUG] Ignored: Group ${groupId} is not in monitored list.`);
            return;
        }
        if (!groupConfig.enable) {
            if (config.debug)
                logger.debug(`[DEBUG] Ignored: Monitoring disabled for group ${groupId}.`);
            return;
        }
        ...
        if (messageId) {
            try {
                ...
                if (config.debug)
                    logger.debug(`[Group: ${groupId}] Message ${messageId} recalled.`);
            }
            catch (err) {
        ...
        if (!record) {
            ...
            if (now - record.firstTime > triggerWindow) {
                if (config.debug)
                    logger.debug(`[Group: ${groupId}] [User: ${userId}] Window expired. Reset count.`);
                record.count = 1;
